...
def _fetch_ohlcv_single(yahoo_ticker: str) -> dict | None:
    ...
    Was 5y, which was overwriting the Kite 7y backfill every day. Now 20y daily —
    Yahoo silently supports 20y (not in their docs but tested). `max` returns
    only ~366 weekly-aggregated bars so we avoid it."""
    url = f"{YAHOO_BASE}/{yahoo_ticker}"
    params = {"interval": "1d", "range": "20y"}
    try:
